// src/app/onboarding/step-3/AthleteStep3Form.tsx
'use client'

import { useRouter } from 'next/navigation'
import { useState } from 'react'

type AthleteStep3Values = {
  name: string
  age: string
  country: string
  language: string
...
export default function AthleteStep3Form() {
  const router = useRouter()
  const [values, setValues] = useState<AthleteStep3Values>({
    name: '',
    age: '',
    country: '',
    language: 'fr',
  ...
  function update<K extends keyof AthleteStep3Values>(
    key: K,
    value: AthleteStep3Values[K],
  ) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const next: string =
        typeof data.next === 'string' ? data.next : '/messages'

      router.push(next)
    } catch (err) {
      console.error('Erreur AthleteStep3Form', err)
      setError(
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <header className="space-y-1">
        <h1 className="text-xl font-semibold text-slate-900">
          Finalise ton profil athlète
        </h1>
        <p className="text-sm text-slate-500">
          Ces informations aident les coachs à mieux te connaître et à
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-slate-900 focus:bg-white"
          placeholder="Parle de ton niveau, ton sport principal, ce que tu recherches..."
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-slate-900 focus:bg-white"
          placeholder="course à pied, trail, premier marathon..."
        />
      </div>
      ...
      <div className="flex items-center justify-between gap-4 pt-2">
        <p className="text-xs text-slate-500">
          Tu pourras modifier ces informations plus tard dans ton compte.
        </p>
