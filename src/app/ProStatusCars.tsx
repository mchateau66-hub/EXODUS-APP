...
type UiState =
  ...
  | { kind: 'unauthorized' }
  | { kind: 'ready'; claim: EntitlementsClaim }
...
function getStatusLabel(claim: EntitlementsClaim) {
  const sub = claim.subscription
  if (!sub) {
    return {
      label: 'Aucun abonnement Pro',
      tone: 'danger' as const,
      helper: 'Passez au plan Master ou Premium pour débloquer les fonctionnalités Pro.'
    }
  }
  ...
  if (sub.status === 'canceled' || sub.status === 'unpaid') {
    return {
      ...
      tone: 'danger' as const,
      helper: 'Votre abonnement a été annulé ou suspendu. Vérifiez votre paiement.'
    }
  }

  // actif, trialing, past_due
  const isSoon =
  ...
  if (isSoon) {
    return {
      ...
      helper: 'Votre période actuelle se termine bientôt. Pensez à vérifier votre moyen de paiement.'
    }
  }
...
export default function ProStatusCard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setState({ kind: 'ready', claim: data.claim })
      } catch (e) {
        if (cancelled) return
        setState({
          kind: 'error',
  ...
  const { claim } = state
  const { label, tone, helper } = getStatusLabel(claim)
  const styles = toneClasses(tone)

  const expiresAt =
    claim.subscription?.expires_at || claim.subscription?.current_period_end
    ...
    >
        ...
        <div className="space-y-2">
          ...
          <h2 className="text-xl font-semibold text-slate-900">
            ...
            <span className="capitalize">
              {claim.plan === 'free' ? 'Free' : claim.plan}
            </span>
          </h2>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl bg-slate-50 px-4 py-3">
          ...
          <div className="mt-1 text-sm font-semibold text-slate-900">
            {claim.subscription?.status ?? 'none'}
          </div>
          <div className="mt-1 text-xs text-slate-500">
        ...
        <div className="rounded-xl bg-slate-50 px-4 py-3">
          ...
          <div className="mt-1 text-sm font-semibold text-slate-900">
            {claim.features.length}
          </div>
          <div className="mt-1 text-xs text-slate-500">
            Calculé depuis <code>user_effective_entitlements</code>.
          </div>
        </div>
      </div>
