import {
  ...
  // ✅ endpoints login/logout configurables (par défaut: endpoints E2E)
  const LOGIN_PATH = (process.env.E2E_LOGIN_PATH?.trim() ?? "/api/e2e/login") as string;
  const LOGOUT_PATH = (process.env.E2E_LOGOUT_PATH?.trim() ?? "/api/e2e/logout") as string;
  ...
  export async function login(
    ...
    const res = await page.request.post(LOGIN_PATH, { data });
  
    if (res.status() >= 400) {
      const body = await res.text().catch(() => "");
      console.log(`[e2e] ${LOGIN_PATH} failed status=${res.status()} url=${res.url()}`);
      if (body) console.log(`[e2e] body: ${body.slice(0, 800)}`);
    }
  
    // ✅ accepte 2xx/3xx (ex: 302) mais refuse 4xx/5xx
    expect(res.status(), `${LOGIN_PATH} status=${res.status()}`).toBeLessThan(400);
    return res;
  }
  
  export async function logout(page: Page): Promise<APIResponse> {
    const res = await page.request.post(LOGOUT_PATH);
  
    if (res.status() >= 400) {
      const body = await res.text().catch(() => "");
      console.log(`[e2e] ${LOGOUT_PATH} failed status=${res.status()} url=${res.url()}`);
      if (body) console.log(`[e2e] body: ${body.slice(0, 800)}`);
    }
  
    expect(res.status(), `${LOGOUT_PATH} status=${res.status()}`).toBeLessThan(400);
    return res;
  }
