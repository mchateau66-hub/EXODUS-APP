...
function getLimiter(name: string, limitN: number, windowMs: number) {
  const k = `${name}:${limitN}:${windowMs}`;
  const existing = rlCache.get(k);
  if (existing) return existing;

  const created = new Ratelimit({
    redis: redis!,
    limiter: Ratelimit.slidingWindow(limitN, `${Math.ceil(windowMs / 1000)} s`),
    analytics: false,
    prefix: `rl:${name}`,
  });
...
function memLimit(name: string, key: string, limitN: number, windowMs: number): RateResult {
  ...
  if (!entry || entry.reset <= now) {
    ...
    return { ok: true, limit: limitN, remaining: limitN - 1, reset: Math.ceil((now + windowMs) / 1000) };
  }

  if (entry.count < entry.limit) {
    entry.count += 1;
    return { ok: true, limit: entry.limit, remaining: entry.limit - entry.count, reset: Math.ceil(entry.reset / 1000) };
  }

  return { ok: false, limit: entry.limit, remaining: 0, reset: Math.ceil(entry.reset / 1000) };
}

export async function limit(name: string, key: string, limitN: number, windowMs: number): Promise<RateResult> {
  ...
  try {
    ...
    // Si Redis tombe, on ne casse pas l’endpoint
